...
/**
 * Returns the enter listener for the given key
 *
 * This listener is invoked when the field gains focus.
 *
 * If there is no listener for the given key, it returns nullptr.
 *
 * @param key   The identifier for the listener
 ...
 * @return the listener for the given key
 */
const TextField::Listener TextField::getEnterListener(Uint32 key) const {
    auto item = _enterListeners.find(key);
    if (item == _enterListeners.end()) {
        return nullptr;
    }
    return item->second;
}
...
/**
 * Returns all enter listeners for this text field
 *
 * These listeners are invoked when the gains focus.
 *
 * @return the enter listeners for this text field
 */
const std::vector<TextField::Listener> TextField::getEnterListeners() const {
    std::vector<Listener> result;
    result.reserve(_enterListeners.size());
    for (auto kv : _enterListeners) {
      result.push_back(kv.second);
    }
    return result;
}
...
/**
 * Returns a key for a enter listener after adding it to this field.
 *
 * This listener is invoked when the field gains focus.
 *
 * C++ cannot hash functions types.  Therefore, the listener will
 ...
 * @return the key for the listener
 */
Uint32 TextField::addEnterListener(Listener listener) {
    CUAssertLog(_nextKey < (Uint32)-1, "No more available listener slots");
    _enterListeners[_nextKey++] = listener;
    return _nextKey;
}

/**
 * Removes a listener from this text field enter.
 *
 * This listener is invoked when the field gains focus.
 *
 * Listeners must be identified by the key returned by the {@link #addListener}
 ...
 * @return true if the listener was succesfully removed
 */
bool TextField::removeEnterListener(Uint32 key) {
    if (_enterListeners.find(key) == _enterListeners.end()) {
        return false;
    }
    _enterListeners.erase(key);
    return true;
}
 ...
 * Clears all listeners for this text field.
 *
 * These listener are invoked when the field gains focus. This
 * method does not require you to remember the keys assigned to
 * the individual listeners.
 */
void TextField::clearEnterListeners() {
    _enterListeners.clear();
}
...
bool TextField::requestFocus() {
	...
	updateCursor();
    invokeListeners(TextField::ListenerType::ENTER);
    return true;
}
...
bool TextField::releaseFocus() {
    ...
    invokeListeners(TextField::ListenerType::EXIT);
    _focused = false;
    return true;
}
...
void TextField::updateInput(const TextInputEvent& event, bool focus) {
    ...
    if (!_font->hasGlyph(event.unicode)) {
        ...
        invokeListeners(TextField::ListenerType::TYPE);
        return;
    }
...
void TextField::updateCursor() {
    ...
    cursorPos = row->exterior.origin;
    if (_cursorIndex >= row->end && !track) {
        cursorPos.x += row->exterior.size.width;
...
size_t TextField::skipWord(bool forward) const {
    ...
    if (forward) {
        ...
        UnicodeType type = strtool::getUnicodeType(code);
        bool tospace = type == UnicodeType::CHAR || type == UnicodeType::CJK;
        bool atend = false;
        ...
        return atend ? cursor-begin-1 : cursor-begin;
    } else {
        if (_cursorIndex == 0) {
            return _cursorIndex;
        }
        ...
        return atbegin ? cursor-begin+1 : 0;
    }
}
 ...
 */
void TextField::invokeListeners(TextField::ListenerType listenerType) {
  switch (listenerType) {
    case TextField::ListenerType::ENTER:
      for (auto it = _enterListeners.begin(); it != _enterListeners.end(); ++it) {
        it->second(getName(), _layout->_text);
      }
      break;
    case TextField::ListenerType::TYPE:
      for (auto it = _typeListeners.begin(); it != _typeListeners.end(); ++it) {
        it->second(getName(), _layout->_text);
      }
      break;
    case TextField::ListenerType::EXIT:
