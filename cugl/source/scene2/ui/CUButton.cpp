...
bool Button::activate() {
    ...
    if (mouse) {
        ...
        bool down = mouse->addPressListener(_inputkey, [=](const MouseEvent& event, Uint8 clicks, bool focus) {
            if (this->inContentBounds(event.position)) {
                if (this->_toggle) {
 ...
 */
bool Button::inContentBounds(const Vec2 point) {
    Vec2 local = screenToNodeCoords(point);
    if (_bounds.size() > 0 && _bounds.contains(local)) {
        if (_parent != nullptr) {
            return _parent->inContentBounds(point);
        }
        return true;
    } else if (Rect(Vec2::ZERO, getContentSize()).contains(local)) {
        if (_parent != nullptr) {
            return _parent->inContentBounds(point);
        }
        return true;
