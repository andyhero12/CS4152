 ...
 *
 * This is the function that ATK uses to filter the result
 */
static size_t filterCallback(void*  userdata, Uint8* stream, size_t len) {
    AudioOutput* device = (AudioOutput*)userdata;
    float* buffer = (float*)stream;
    Uint32 frames = (Uint32)(len/(device->getChannels()*sizeof(float)));
    return device->read(buffer,frames);
}
 ...
 */
static size_t format_bytes(SDL_AudioFormat fmt) {
    switch (fmt) {
    case AUDIO_S8:
    case AUDIO_U8:
        return 1;
...
bool AudioOutput::reopenDevice() {
    ...
    _device = SDL_OpenAudioDevice((_dvname == "" ? NULL : _dvname.c_str()),
                                  0, &_wantspec, &_audiospec, SDL_AUDIO_ALLOW_ANY_CHANGE);
    if (_device == 0) {
    ...
    bool convert = false;
    convert = _wantspec.freq != _audiospec.freq || convert;
    convert = _wantspec.channels != _audiospec.channels || convert;
    convert = _wantspec.format != _audiospec.format || convert;
    
    if (convert) {
        _cvtfilter = ATK_AllocAudioCVT(&_wantspec, &_audiospec, filterCallback);
    }
...
Uint32 AudioOutput::poll(Uint8* stream, int len) {
    Uint32 take;
    if (_cvtfilter == NULL) {
        Uint32 frames = len/(_audiospec.channels*format_bytes(_audiospec.format));
        float* buffer = (float*)stream;
        take = read(buffer,frames);
        if (take < frames) {
            std::memset(buffer+take*_audiospec.channels,0,(frames-take)*_audiospec.channels);
        }
        take = (Uint32)(take*_audiospec.channels*format_bytes(_audiospec.format));
    } else {
        take = (Uint32)filter(stream,len);
        if (take < len) {
            std::memset(stream+take,0,len-take);
        }
    }
    return take;
}
 ...
 * of this AudioOutput.
 *
 * This method is used by the ATK library to convert audio from one
 * filter to another. It should never be called by the developer.
 *
 * @param stream    The read buffer to store the results
 * @param len       The maximum number of bytes to read
 *
 * @return the actual number of bytes read
 */
size_t AudioOutput::filter(Uint8* stream, size_t len) {
    if (_cvtfilter == NULL) {
        Uint32 frames = (Uint32)(len/(sizeof(float)*_channels));
        return read((float*)stream,frames);
    }
    
    Sint64 taken = ATK_PollAudioCVT(_cvtfilter, stream, (int)len);
